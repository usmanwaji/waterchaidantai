...
// อำเภอลุ่มน้ำคลองอู่ตะเภา (หาดใหญ่ + ต้นน้ำที่ให้สัญญาณเตือนล่วงหน้า 6–12 ชม.)
const BASIN_AMPHOE = new Set(['หาดใหญ่','สะเดา','คลองหอยโข่ง','นาหม่อม','บางกล่ำ','ควนเนียง']);
const PROV_SET = new Set(PROVINCES.map(p=>p.name));
const BBOX = {latMin:6.30, latMax:7.35, lonMin:99.95, lonMax:100.75};   // ลุ่มน้ำคลองอู่ตะเภา อ.หาดใหญ่ + อำเภอต้นน้ำ จ.สงขลา
const inBbox = (lat,lon)=> lat!=null && lon!=null && lat>=BBOX.latMin && lat<=BBOX.latMax && lon>=BBOX.lonMin && lon<=BBOX.lonMax;
// อยู่ในลุ่มน้ำ: อำเภอตรงชุด BASIN_AMPHOE หรือ (ไม่ทราบอำเภอแต่พิกัดอยู่ใน BBOX)
const inBasin = (amphoe,lat,lon)=> amphoe ? BASIN_AMPHOE.has(String(amphoe).trim()) : inBbox(lat,lon);

/* ================= ภาษา (TH / EN / MS) ================= */
const STR = {
th:{
  title:'🌊 One Map ระดับน้ำหาดใหญ่', sub:'อ.หาดใหญ่ จ.สงขลา · ลุ่มน้ำคลองอู่ตะเภา — รวมทุกหน่วยงานในแผนที่เดียว',
  ...
  rpTitle:'📋 รายงานสถานการณ์น้ำ อ.หาดใหญ่ จ.สงขลา (ลุ่มน้ำคลองอู่ตะเภา)', rpStations:'สถานี', rpCrit:'⚠ สถานีเฝ้าระวังเร่งด่วน:', rpNoCrit:'✅ ไม่มีสถานีล้นตลิ่ง/น้ำมาก', rpRainMax:'🌧 ฝนสะสม 24 ชม. สูงสุด:', rpDam:'🔻 เขื่อน/อ่างเก็บน้ำ:', rpRisk:'⚠ พื้นที่เสี่ยงน้ำท่วมฉับพลัน:', rpSrc:'ที่มา: thaiwater.net (สสน.) / HII',
...
en:{
  title:'🌊 One Map — Hat Yai Water Watch', sub:'Hat Yai District, Songkhla · Khlong U-Taphao basin — all agencies, one map',
  ...
  rpTitle:'📋 Water situation report — Hat Yai, Songkhla (U-Taphao basin)', rpStations:'stations', rpCrit:'⚠ Priority stations:', rpNoCrit:'✅ No overflow/high stations', rpRainMax:'🌧 Max rain 24 h:', rpDam:'🔻 Dams/reservoirs:', rpRisk:'⚠ Flash-flood risk areas:', rpSrc:'Source: thaiwater.net (HII)',
...
ms:{
  title:'🌊 One Map — Paras Air Hat Yai', sub:'Daerah Hat Yai, Songkhla · Lembangan Khlong U-Taphao — semua agensi, satu peta',
  ...
  rpTitle:'📋 Laporan situasi air — Hat Yai, Songkhla (lembangan U-Taphao)', rpStations:'stesen', rpCrit:'⚠ Stesen keutamaan:', rpNoCrit:'✅ Tiada stesen melimpah/tinggi', rpRainMax:'🌧 Hujan 24 j tertinggi:', rpDam:'🔻 Empangan:', rpRisk:'⚠ Kawasan risiko banjir kilat:', rpSrc:'Sumber: thaiwater.net (HII)',
...
/* ================= แผนที่ ================= */
const map = L.map('map', {zoomControl:false}).setView([7.01, 100.46], 12);
...
function shareSnapshot(){
  ...
  x.fillStyle='#fff'; x.font='bold 23px sans-serif'; x.fillText('🌊 สรุปสถานการณ์น้ำ หาดใหญ่', 20, 39);
...
let markersById = {};
let filterProv = 'หาดใหญ่';   // กรองรายอำเภอในลุ่มน้ำ
...
async function loadWaterLevel(){
  ...
  rows.forEach(d => {
    ...
    if(!inBasin(thName(d.geocode?.amphoe_name), lat, lon)) return;
...
const BND_NAMES = ['สงขลา','Songkhla'];
...
function renderSidebar(){
  ...
  const inProv = wlStations.filter(s => filterProv==='all' || s.amphoe===filterProv);
  const counts = {};
...
const PROV_VIEWS = {
  'all':[[6.85,100.40],10],
  'หาดใหญ่':[[7.01,100.46],12],
  'สะเดา':[[6.64,100.42],11],
  'คลองหอยโข่ง':[[6.87,100.34],12],
  'นาหม่อม':[[6.96,100.56],12],
  'บางกล่ำ':[[7.05,100.39],12],
  'ควนเนียง':[[7.17,100.36],12]
};
